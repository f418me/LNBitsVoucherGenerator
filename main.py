...
def createPage(pageCounter, idList):
  ...
  for xCord in qrXCoord:
    frontImage = Image.open(config.get('General', 'OutputDirectory') + '/' + 'qr_Page_' + str(pageCounter) + '_'+ str(currentQROnPage) + '.png')
...
filename= config.get('General', 'OutputDirectory') + "/" + config.get('Voucher', 'NameOfVoucherBatch') + "_Vouchers.pdf"
